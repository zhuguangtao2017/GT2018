...
class GoodsController extends BaseController {
    ...
    public function categoryList(){
        ...
        $cat_list = M('goods_category')->where(['parent_id'=>0,'level'=>1])->select();
        $this->assign('cat_list',$cat_list);
        $this->display();
    }


    public function kucunlist()
    {
        $xxx = 10;
        $count = M('spec_goods_price')
            ->alias('a')
            ->field('a.goods_id,a.key_name,ty_goods.goods_name')
            ->join('ty_goods')
            ->where("a.store_count<$xxx and a.goods_id = ty_goods.goods_id ")
            ->select();
        $this -> assign('brandList',$count);
        $this -> display();
    }
    public function addEditCategorydel(){
        if(is_array($_POST['cat_id'])){
            //dump($_POST['cat_id']);
            $_POST['cat_id'] = implode(',',$_POST['cat_id']);
            M("GoodsCategory")->delete($_POST['cat_id']);
            return $this->success("操作成功!!!",U('Admin/Goods/categoryList'));
        }
    }
    public function ajaxcate(){
        if(!empty(I('id'))){
            $lv2 = M('goods_category')->where(['parent_id'=>I('id')])->select();
            foreach ($lv2 as $key=> $v){
                if($v['level'] == 2) $lv2[$key]['level2'] = 'Ⅱ';
                else $lv2[$key]['level2'] = 'Ⅲ';
            }
            exit(json_encode(['status'=>1,'msg'=>'获取成功','data'=>$lv2]));
        }
    }
     ...
     * select * from tp_goods_category where id = 393
        ...
        ('393','时尚饰品'),
     */
    public function addEditCategory1(){
    ...
    public function Adel(){
        ...
        if(!$GoodsCategory->create(NULL,$type))// 根据表单提交的POST数据创建数据对象
        {
            ...
            //  form表单提交
            C('TOKEN_ON', true);
            $GoodsCategory->name = I('name');
            $GoodsCategory->mobile_name = I('mobile_name');
            if(!empty(I('id'))){ $_POST['id'] = I('id');}
            else $_POST['id'] = $GoodsCategory->add();
            if(I('type') == 1){
                $level = 1;                         //添加为品类
                $parent_id = 0;
                $parent_id_path = '0_' . $_POST['id'];
            }elseif (I('type') == 2){                       //添加为品牌
                $level = 2;
                $parent_id = I('cat_id1');
                $parent_id_path = '0_' . I('cat_id1') . '_' . $_POST['id'];
            }else{                                           //添加为品类
                if (empty(I('cat_id2'))) {
                    $parent_id = I('cat_id1');
                    $parent_id_path = '0_' . I('cat_id1') . '_' . $_POST['id'];
                    ...
                    $parent_id = I('cat_id2');
                    $parent_id_path = '0_' . I('cat_id1') . '_' . I('cat_id2') . '_' . $_POST['id'];
                }
                $level = 3;
            }
            M('goods_category')->where("id={$_POST['id']}")->save(['parent_id_path' => $parent_id_path, 'level' => $level,'parent_id'=>$parent_id,'mobile_name'=>I('mobile_name'),'image'=>I('image'),'content'=>I('content'),'module_nimg'=>I('module_nimg'),'text1'=>I('text1'),'text2'=>I('text2'),'cart1'=>I('cart1')]);
            $return_arr = array(
                'status' => 1,
                'msg' => '添加成功',
                'url' => U('Admin/Goods/categoryList'),
            );
            exit(json_encode($return_arr));
        }
    }
    public function addEditCategory(){
        ...
        if(IS_GET)
        {
            ...
            $cat_list = M('goods_category')->where("parent_id = 0 and level = 1")->select(); // 已经改成联动菜单
            $this->assign('level_cat',$level_cat);
            $this->assign('cat_list',$cat_list);
            $this->assign('goods_category_info',$goods_category_info);
            ...
            $goods_type_list = M('goods_type')->select(); // 所有类型id
            $this->assign('goods_type_list',$goods_type_list);
            $mobil = M('module')->select();
            $this->assign('mobil',$mobil);
            $this->initEditor();
            $this->display('_category');
            exit;
        }
    ...
    public function ajaxSx(){
        if($_POST['type'] == 1){
            $html = '<option value="">请选择...</option><option value="1">商品上架</option><option value="3">违规下架</option>';
            exit($html);
        }
        if($_POST['type'] == 2){
            $goods_state = C('goods_state');
                $html = '<option value="">请选择...</option><option value="1">审核通过</option><option value="2">审核失败</option><option value="3">违规下架</option>';
            exit($html);
        }
    }
    /**
    ...
    public function ajaxGoodsList(){            
        ...
        I('cat_id2') && $where = "$where and cat_id2 = ".I('cat_id2') ;
        I('cat_id3') && $where = "$where and cat_id3 = ".I('cat_id3') ;
        I('module_id') && $where = "$where and module_id = ".I('module_id') ;
        ...
        if(I('type')==1) $where.=" and store_id = 0";
        else $where.=" and store_id != 0 and sellType = 5";
    ...
    public function addmodule(){
        $id = I('get.id');
        if(!empty($id)){
            $info = M('module')->find($id);
            $this->assign('info',$info);
        }
        if(!empty(I('post.module_name'))){
            if(empty(I('module_id'))){
                $_POST['create_time'] = time();
                M('module')->add($_POST);
                $return_arr = array(
                    'status' => 1,
                    'msg'   => '操作成功',
                    'data'  => array('url'=>U('Admin/Goods/module')),
                );
                $this->ajaxReturn(json_encode($return_arr));
            }else{
                $_POST['create_time'] = time();
//                dump($_POST); die;
    ...
    public function delmodule(){
        if(!empty(I('id'))){
            $ar = M('module')->find(I('id'));
            $count = M('goods')->where(['module_id'=>I('id')])->count();
            if($count>0) exit(json_encode(['status'=>-1,'msg'=>'该模块下存在商品']));
            $img = ROOT_PATH .$ar['module_img'];
            if(file_exists($img)){
                unlink($img);
            }
            $result = M('module')->delete(I('id'));
            if($result){
                exit(json_encode(['status'=>1,'msg'=>'操作成功']));
            }else{
                exit(json_encode(['status'=>-1,'msg'=>'网络错误']));
            }
        }
    }
    /**
    ...
    public function addEditGoods(){
            ...
            if(($_GET['is_ajax'] == 1) && IS_POST)
            {
                $count = 0;
                C('TOKEN_ON',false);
                $lengt = count($_POST['name']);
                for($i = 0;$i<$lengt;$i++){
                    $count+= $_POST['count'][$i];
                }
                $_POST['store_count'] = $count;
                $_POST['cost_price'] = $_POST['shop_price'];
                if(!file_exists(ROOT_PATH.$_POST['original_img'])){
                    $_POST['original_img'] = $_POST['goods_images'][0];
                }
//                $_POST['vip_price'] = $_POST['money'][0];
                    ...
                    if ($type == 2)
                    {
                        ...
                        for($i = 0;$i<$lengt;$i++){
                            $data = [
                                'goods_id' => $goods_id,
                                'key' => $key.$i,
                                'key_name' => $_POST['name'][$i],
                                'store_count' => $_POST['count'][$i],
                                'price' => $_POST['price'][$i],
//                                'money' => $_POST['money'][$i],
                            ];
                            $res = M('spec_goods_price')-> add($data);
                        }
...
//                                'money' => $_POST['money'][$i],
            ...
            $goodsPrice = M("spec_goods_price")->where('goods_id ='.I('GET.id',0))->select();
            // $goodsImages = M("GoodsImages")->where(['goods_id' => 123])->select();
            $list = M('module')->select();
            $this->assign('module',$list);
            // var_dump($goodsImages);die;
            $this->assign('goodsImages',$goodsImages);  // 商品相册
            $this->assign('goodsPrice',$goodsPrice);  // 商品相册
            $this->initEditor(); // 编辑器
            $this->display('_goods');                                     
    }  
    ...
    public function specList(){               
        $cat_list = M('goods_category')->where("parent_id = 0")->getField('id,mobile_name,parent_id'); // 已经改成联动菜单
        $this->assign('cat_list',$cat_list);        
        $this->display();
    }
    ...
    public function ajaxSpecList(){ 
        ...
        $cat_list = M('goods_category')->where("parent_id = 0")->getField('id,mobile_name'); // 已经改成联动菜单
...
//        var_dump($cat_list);
    ...
    public function act()
    {
        ...
        $goods_state == 1 ? $is_on_sale = 1 : $is_on_sale = 0 ;
        ...
        if ($act = 'examine') {
            ...
            foreach ($goods_array as $key => $val) {
                $update_goods_state = M('goods')->where("goods_id = $val")->save(array('goods_state' => $goods_state,'is_on_sale' => $is_on_sale));
