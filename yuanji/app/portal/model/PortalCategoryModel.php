...
//        }
	...
	public function adminCategoryTable1Tree($uid,$pid,$currentIds = 0, $tpl = '')
    {
        //$where = ['delete_time' => 0];
//        if (!empty($currentCid)) {
//            $where['id'] = ['neq', $currentCid];
//        }
		$UM=new UserModel();
		$xiaxianid=$UM->getSons([$uid],$pid,4);//下线id
		array_push($xiaxianid,$uid);
		$UM->banArr();
        $categories = Db::name("user")->where(['id'=>["in",$xiaxianid]])->order("id ASC")->select()->toArray();
		//print_r($xiaxianid);
		//print_r($categories);
		//echo "<br>";
		//die;
//$categories = Db::name("user")->where("pid",5)->order("id ASC")->select()->toArray();
//print_r($categories);
//die();
        $tree       = new Tree();
        $tree->icon = ['&nbsp;&nbsp;│', '&nbsp;&nbsp;├─', '&nbsp;&nbsp;└─'];
        ...
        foreach ($categories as $item) {
            ...
            $item['parent_id'] = $item["fid"];
			$item["name"] = $item["user_nickname"];			
           $item["url"] =cmf_url('user/profile/xiaxian', ['id' => $item['id']]);

			$money=Db::name("pay")->where("uid",$item["id"])->sum("money");
			$item["moneys"]=$money;
            array_push($newCategories, $item);
        }
        ...
        if (empty($tpl)) {
            $tpl = "<tr>
                        <td>\$id</td>
                        <td>\$spacer <a href='\$url' target='_blank'>\$name</a>-- 消费 ￥\$moneys</td>             </tr>";
        }
        $treeStr = $tree->getTree(0, $tpl);
	...
	public function zjh($pid,$currentIds = 0, $tpl = '')
    {
		
		$categories = Db::name("user")->where("pid",$pid)->order("id ASC")->select()->toArray();
        $tree       = new Tree();
        $tree->icon = ['&nbsp;&nbsp;│', '&nbsp;&nbsp;├─', '&nbsp;&nbsp;└─'];
        $tree->nbsp = '&nbsp;&nbsp;';
        ...
        foreach ($categories as $item) {
			...
			$item["user_type"] = "<span style='color:".$color[$item["user_type"]]."'>".$arr[$item["user_type"]].'</span>';

            $item["url"] =cmf_url('//user/user_order/index', ['name' => $item['id']]);
        ...
        $tree->init($newCategories);
//\$spacer <a href='\$url' target='_blank'>\$name</a>--￥\$moneys
        if (empty($tpl)) {
            $tpl = "<tr>
                        <td>\$spacer <a href='\$url'>\$name</a> \$user_type 消费金额￥\$moneys</td>
					</tr>";
        }
        $treeStr = $tree->getTree(0, $tpl);
